...
namespace Turbo.Runtime
{
    internal sealed class ParameterDeclaration : ParameterInfo
    {
        ...
        internal ParameterDeclaration(Context context, string identifier, TypeExpression type,
            CustomAttributeList customAttributes)
        {
            ...
            {
                context.HandleError(TError.DuplicateName, this.identifier,
                    activationObject is ClassScope || activationObject.fast || type != null);
            }
            else
            {
                activationObject.AddNewField(this.identifier, null, FieldAttributes.PrivateScope).originalContext =
                    context;
            }
