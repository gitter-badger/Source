	...
	public sealed class THPMainEngine : THPEngine, ITEngine, IRedirectOutput
	{
	    ...
	    private TypeReferences TypeRefs
		{
			get
			{
				...
				{
					typeReferences = _reflectionOnlyTypeRefs ?? (_reflectionOnlyTypeRefs = new TypeReferences(TurboModule));
				}
				else
				{
		...
		public LenientGlobalObject LenientGlobalObject
		{
			get
			{
				return (LenientGlobalObject)Globals.globalObject;
			}
		}
		...
		{
			return string.Concat(14, ".", 0.ToString(CultureInfo.InvariantCulture).PadLeft(2, '0'), ".", 0.ToString(CultureInfo.InvariantCulture), ".", 79.ToString(CultureInfo.InvariantCulture).PadLeft(4, '0'));
		}
		...
		internal static void EnsureReflectionOnlyModulesLoaded()
		{
		    ...
		    reflectionOnlyVsaModule = Assembly.ReflectionOnlyLoadFrom(typeof(ITHPEngine).Assembly.Location).GetModule("Turbo.Runtime.dll");
		    reflectionOnlyTurboModule = Assembly.ReflectionOnlyLoadFrom(typeof(THPMainEngine).Assembly.Location).GetModule("Turbo.Runtime.dll");
		}
		...
		internal void CheckForErrors()
		{
			if (!isClosed && !isEngineCompiled)
			{
				...
				try
				{
					foreach (var current in vsaItems.OfType<THPReference>())
					{
					    current.Compile();
					}
					if (vsaItems.Count > 0)
					...
					foreach (var current2 in vsaItems.Cast<object>().Where(current2 => !(current2 is THPReference)))
					{
		...
		{
			var vsaEngine = new THPMainEngine(fast);
			vsaEngine.InitTHPMainEngine("Turbo.Vsa.THPMainEngine://Turbo.Runtime.THPMainEngine.Vsa", new THPDefaultSite());
			vsaEngine.doPrint = true;
			vsaEngine.SetEnclosingContext(new WrappedNamespace("", vsaEngine));
			foreach (var text in assemblyNames)
			{
			    ((THPReference)vsaEngine.vsaItems.CreateItem(text, ETHPItemType.Reference, ETHPItemFlag.None)).AssemblyName = text;
			}
			exeEngine = vsaEngine;
			var expr_74 = (GlobalScope)vsaEngine.GetGlobalScope().GetObject();
			expr_74.globalObject = vsaEngine.Globals.globalObject;
			return expr_74;
		}
		...
		public static GlobalScope CreateEngineAndGetGlobalScopeWithTypeAndRootNamespace(bool fast, string[] assemblyNames, RuntimeTypeHandle callingTypeHandle, string rootNamespace)
		{
			...
			System.Runtime.Remoting.Messaging.CallContext.SetData("Turbo:" + assembly.FullName, vsaEngine);
			var expr_A1 = (GlobalScope)vsaEngine.GetGlobalScope().GetObject();
			expr_A1.globalObject = vsaEngine.Globals.globalObject;
			return expr_A1;
		}
		...
		{
			var vsaEngine = new THPMainEngine(true);
			vsaEngine.InitTHPMainEngine("Turbo.Vsa.THPMainEngine://Turbo.Runtime.THPMainEngine.Vsa", new THPDefaultSite());
			((GlobalScope)vsaEngine.GetGlobalScope().GetObject()).globalObject = vsaEngine.Globals.globalObject;
			return vsaEngine;
		}
		...
		public static THPMainEngine CreateEngineWithType(RuntimeTypeHandle callingTypeHandle)
		{
			...
			if (data != null)
			{
				var vsaEngine = data as THPMainEngine;
				if (vsaEngine != null)
				{
					return vsaEngine;
				}
			}
			var vsaEngine2 = new THPMainEngine(assembly);
			vsaEngine2.InitTHPMainEngine("Turbo.Vsa.THPMainEngine://Turbo.Runtime.THPMainEngine.Vsa", new THPDefaultSite());
			var globalScope = (GlobalScope)vsaEngine2.GetGlobalScope().GetObject();
			...
			if (data == null)
			{
				System.Runtime.Remoting.Messaging.CallContext.SetData("Turbo:" + assembly.FullName, vsaEngine2);
			}
			return vsaEngine2;
		}
		...
		private void AddReferences()
		{
			...
			if (array != null)
			{
			    ...
			    {
			        ((THPReference)vsaItems.CreateItem(t, ETHPItemType.Reference, ETHPItemFlag.None)).AssemblyName = t;
			    }
			    return;
			}
		    var customAttributes = CustomAttribute.GetCustomAttributes(runtimeAssembly, typeof(ReferenceAttribute), false);
			...
			for (var j = 0; j < customAttributes.Length; j++)
			{
				...
				((THPReference)vsaItems.CreateItem(reference, ETHPItemType.Reference, ETHPItemFlag.None)).AssemblyName = reference;
				array2[j] = reference;
			}
		...
		{
			var simpleHashtable = new SimpleHashtable((uint)(vsaItems.Count + (implicitAssemblies?.Count ?? 0)));
			foreach (var current in vsaItems)
			{
			    ...
			    var customAttribute = new CustomAttributeBuilder(CompilerGlobals.referenceAttributeConstructor, new object[]
			    {
			        fullName
			    });
		    ...
		    foreach (var current2 in implicitAssemblies)
		    {
		        ...
		        var customAttribute2 = new CustomAttributeBuilder(CompilerGlobals.referenceAttributeConstructor, new object[]
		        {
		            fullName2
		        });
		...
		private void CreateMain()
		{
			...
			var methodBuilder = typeBuilder.DefineMethod("Main", MethodAttributes.FamANDAssem | MethodAttributes.Family | MethodAttributes.Static, Typeob.Void, new[]
			{
		...
		{
			var typeBuilder = CompilerGlobals.module.DefineType(rootNamespace + "._Startup", TypeAttributes.Public, Typeob.THPStartup);
			var field = Typeob.THPStartup.GetField("site", BindingFlags.Instance | BindingFlags.NonPublic);
			var methodBuilder = typeBuilder.DefineMethod("Startup", MethodAttributes.FamANDAssem | MethodAttributes.Family | MethodAttributes.Virtual, Typeob.Void, Type.EmptyTypes);
			CreateEntryPointIL(methodBuilder.GetILGenerator(), field, typeBuilder);
			var methodBuilder2 = typeBuilder.DefineMethod("Shutdown", MethodAttributes.FamANDAssem | MethodAttributes.Family | MethodAttributes.Virtual, Typeob.Void, Type.EmptyTypes);
	    ...
	    private void CreateEntryPointIL(ILGenerator il, FieldInfo site, Type startupClassLoc = null)
		{
	        ...
	        var simpleHashtable = new SimpleHashtable((uint)vsaItems.Count);
			...
			var flag2 = false;
			enumerator = vsaItems.GetEnumerator();
			try
			{
				while (enumerator.MoveNext())
				{
		...
		private void CreateHostCallbackIL(ILGenerator il, FieldInfo site)
		{
			...
			foreach (var vsaHostObject in vsaItems.OfType<THPHostObject>())
			{
			    il.Emit(OpCodes.Ldarg_0);
			    il.Emit(OpCodes.Ldfld, site);
			    il.Emit(OpCodes.Ldstr, vsaHostObject.Name);
			    il.Emit(OpCodes.Callvirt, method);
			    var fieldType = vsaHostObject.Field.FieldType;
		...
		{
		    foreach (var current in vsaItems.OfType<THPHostObject>())
		    {
		        il.Emit(OpCodes.Ldnull);
		...
		protected override bool DoCompile()
		{
			if (!isClosed && !isEngineCompiled)
			{
				...
				try
				{
					...
					foreach (var current2 in vsaItems.OfType<THPHostObject>())
					{
					    current2.Compile();
					}
					foreach (var current3 in vsaItems.OfType<THPStaticCode>())
					{
					    current3.Parse();
					}
					foreach (var current4 in vsaItems.OfType<THPStaticCode>())
					{
					    current4.ProcessAssemblyAttributeLists();
					}
					foreach (var current5 in vsaItems.OfType<THPStaticCode>())
					{
					    current5.PartiallyEvaluate();
					}
					foreach (var current6 in vsaItems.OfType<THPStaticCode>())
					{
					    current6.TranslateToIL();
					}
					foreach (var current7 in vsaItems.OfType<THPStaticCode>())
					{
			...
			if (managedResources != null)
			{
				foreach (TPHResInfo resInfo in managedResources)
				{
					if (resInfo.isLinked)
					{
						CompilerGlobals.assemblyBuilder.AddResourceFile(resInfo.name, Path.GetFileName(resInfo.filename), resInfo.isPublic ? ResourceAttributes.Public : ResourceAttributes.Private);
					}
					else
					{
						try
						{
							...
							{
								var resourceWriter = CompilerGlobals.module.DefineResource(resInfo.name, resInfo.filename, resInfo.isPublic ? ResourceAttributes.Public : ResourceAttributes.Private);
		...
		{
			return ((THPItems)vsaItems).staticCodeBlockCount;
		}
		...
		private void LoadVsaItems(XmlNode parent)
		{
			...
			foreach (XmlElement xmlElement in arg_44_0)
			{
				...
				{
					iJSVsaItem = vsaItems.CreateItem(attribute, ETHPItemType.Reference, ETHPItemFlag.None);
				...
				{
					iJSVsaItem = vsaItems.CreateItem(attribute, ETHPItemType.AppGlobal, ETHPItemFlag.None);
				...
				else
				{
					...
					iJSVsaItem = vsaItems.CreateItem(attribute, ETHPItemType.Code, ETHPItemFlag.None);
		...
		protected override void ResetCompiledState()
		{
			...
			compilerGlobals = null;
			globals = null;
			var enumerator = vsaItems.GetEnumerator();
			try
			{
				while (enumerator.MoveNext())
				{
		...
		private void SaveSourceForDebugging()
		{
			...
			foreach (THPItem vsaItem in vsaItems)
			{
			    if (!(vsaItem is THPStaticCode)) continue;
			    var text = debugDirectory + vsaItem.Name + ".js";
			    try
			    {
			        using (var fileStream = new FileStream(text, FileMode.Create, FileAccess.Write))
			        {
			            ...
			            {
			                streamWriter.Write(((THPStaticCode)vsaItem).SourceText);
			            }
		...
		private void SaveVsaItems(XmlDocument project, XmlNode parent)
		{
			...
			foreach (ITHPItem iJSVsaItem in vsaItems)
			{
				...
				if (iJSVsaItem is THPHostObject)
				{
					...
					CreateAttribute(project, xmlElement2, "ExposeMembers", ((THPHostObject)iJSVsaItem).ExposeMembers.ToString(CultureInfo.InvariantCulture));
				}
		...
		internal void TryToAddImplicitAssemblyReference(string name)
		{
			...
			if (implicitAssemblyCache == null)
			{
				...
				try
				{
					while (enumerator.MoveNext())
					{
						var vsaReference = enumerator.Current as THPReference;
					    if (vsaReference?.AssemblyName == null) continue;
			...
			try
			{
				var vsaReference2 = new THPReference(this, name + ".dll");
			    if (!vsaReference2.Compile(false)) return;
