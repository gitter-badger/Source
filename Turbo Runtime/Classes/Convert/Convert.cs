	...
	public static class Convert
	{
		...
		internal static object Coerce(object value, object type, bool explicitOK)
		{
			...
			if (typedArray != null)
			{
				...
				var elementType2 = elementType is Type ? (Type)elementType : (elementType is ClassScope ? ((ClassScope)elementType).GetBakedSuperType() : typeof(object));
	        ...
	        while (true)
	        {
	            ...
	            else
	            {
	                ...
	                if (value is Array && t == typeof (ArrayObject) && ((Array) value).Rank == 1)
	                {
	                    ...
	                    Globals.contextEngine.InitTHPMainEngine("JS7://Turbo.Runtime.THPMainEngine", new THPDefaultSite());
	                    return Globals.contextEngine.GetOriginalArrayConstructor().ConstructWrapper((Array) value);
	                }
	            ...
	            if (explicitOK)
	            {
	                methodInfo = t.GetMethod("op_Explicit", BindingFlags.Static | BindingFlags.Public | BindingFlags.ExactBinding, null, new[]
	                {
	                    type2
	                }, null);
	            ...
	            methodInfo = t.GetMethod("op_Implicit", BindingFlags.Static | BindingFlags.Public | BindingFlags.ExactBinding, null, new[]
	            {
	                type2
	            }, null);
	            if (methodInfo != null && (methodInfo.Attributes & MethodAttributes.SpecialName) != MethodAttributes.PrivateScope)
	    ...
	    public static object Coerce2(object value, TypeCode target, bool truncationPermitted) 
            => truncationPermitted ? Coerce2WithTruncationPermitted(value, target) : Coerce2WithNoTrunctation(value, target);
			...
			checked
			{
				try
				{
					...
					case TypeCode.String:
					{
						...
						{
							result = DatePrototype.getVarDate(DateConstructor.ob.CreateInstance(DatePrototype.ParseDate(text)));
							return result;
						}
						IL_16B9:
						result = text;
	    ...
	    internal static void Emit(AST ast, ILGenerator il, Type source_type, Type target_type, bool truncationPermitted = false)
	    {
	        while (true)
	        {
		...
		{
			var methodInfo = target_type.GetMethod("op_Explicit", BindingFlags.Static | BindingFlags.Public | BindingFlags.ExactBinding, null, new[]
			{
				source_type
			}, null);
			if (methodInfo != null)
			...
			methodInfo = target_type.GetMethod("op_Implicit", BindingFlags.Static | BindingFlags.Public | BindingFlags.ExactBinding, null, new[]
			{
				source_type
			}, null);
			if (methodInfo != null)
		...
		private static MethodInfo GetToXXXXMethod(IReflect ir, Type desiredType, bool explicitOK)
		{
			...
			var member = ir.GetMember(explicitOK ? "op_Explicit" : "op_Implicit", BindingFlags.Static | BindingFlags.Public);
		    if (member == null) return null;
		    var array = member;
		    return array.Where(memberInfo => memberInfo is MethodInfo && ((MethodInfo) memberInfo).ReturnType == desiredType).Cast<MethodInfo>().FirstOrDefault();
		}
	    ...
	    internal static bool IsArray(IReflect ir) 
            => ReferenceEquals(ir, Typeob.Array) || ReferenceEquals(ir, Typeob.ArrayObject) || ir is TypedArray || ir is ArrayObject || (ir is Type && ((Type)ir).IsArray);

	    internal static bool IsArrayRankKnown(IReflect ir) 
            => ReferenceEquals(ir, Typeob.ArrayObject) || ir is TypedArray || ir is ArrayObject || (ir is Type && ((Type)ir).IsArray);

	    internal static bool IsArrayType(IReflect ir) 
            => ir is TypedArray || ReferenceEquals(ir, Typeob.Array) || ReferenceEquals(ir, Typeob.ArrayObject) || (ir is Type && ((Type)ir).IsArray);
		...
		private static bool IsPromotableTo(Type source_type, Type target_type)
		{
			...
			var methodInfo = target_type.GetMethod("op_Implicit", BindingFlags.Static | BindingFlags.Public | BindingFlags.ExactBinding, null, new[]
			{
				source_type
			}, null);
			if (methodInfo != null && (methodInfo.Attributes & MethodAttributes.SpecialName) != MethodAttributes.PrivateScope)
	    ...
	    internal static bool IsPromotableTo(IReflect source_ir, IReflect target_ir)
	    {
	        while (true)
	        {
	            if (source_ir is TypedArray || target_ir is TypedArray || source_ir is ArrayObject || target_ir is ArrayObject || ReferenceEquals(source_ir, Typeob.ArrayObject) || ReferenceEquals(target_ir, Typeob.ArrayObject))
	            {
		...
		internal static double ToNumber(string str, bool hexOK, bool octalOK, object radix)
		{
		    ...
		    return num4 < length3 ? ToNumber(parseRadix(str.ToCharArray(), (uint)num6, num4, num5, hexOK & octalOK)) : double.NaN;
		}
		...
		private static object ToPrimitive(object value, PreferredType preferredType, IConvertible ic, TypeCode tcode)
		{
			...
			{
			    return tcode != TypeCode.DateTime ? value : DateConstructor.ob.Construct(ic.ToDateTime(null)).GetDefaultValue(preferredType);
			}
		    var array = value as Array;
		    ...
		    {
		        methodInfo = (GetToXXXXMethod(reflect, typeof(double), true) ?? GetToXXXXMethod(reflect, typeof(long), true)) ??
		    ...
		    try
		    {
		        try
		        {
		            ...
		            {
		                var type = value.GetType();
		                if (TypeReflector.GetTypeReflectorFor(type).Is__ComObject() && (!THPMainEngine.executeForJSEE || !(value is IDebuggerObject)))
		                {
		                    reflect = type;
		                }
		            }
		            if (THPMainEngine.executeForJSEE)
		            {
		                ...
		                {
		                    if (!debuggerObject.IsScriptObject()) throw new TurboException(TError.NonSupportedInDebugger);
		                    result = reflect.InvokeMember("< Turbo-" + preferredType + " >", BindingFlags.InvokeMethod | BindingFlags.GetField | BindingFlags.GetProperty | BindingFlags.ExactBinding | BindingFlags.SuppressChangeType, null, value, new object[0], null, null, new string[0]);
		                    return result;
		                }
		            }
		            result = reflect.InvokeMember(string.Empty, BindingFlags.InvokeMethod | BindingFlags.GetField | BindingFlags.GetProperty | BindingFlags.ExactBinding | BindingFlags.SuppressChangeType, null, value, new object[0], null, null, new string[0]);
		            return result;
		        }
