...
namespace Turbo.Runtime
{
	internal sealed class THPNamedItemScope : ScriptObject, IActivationObject
	{
		...
		internal override object GetMemberValue(string name)
		{
		    ...
		    try
		    {
		        var field = reflectObj.GetField(name, BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.FlattenHierarchy);
		        if (field == null)
		        {
		            var property = reflectObj.GetProperty(name, BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.FlattenHierarchy);
		            if (property != null)
		            {
