...
namespace Turbo.Runtime
{
    public class TObject : ScriptObject, IEnumerable, IDynamicElement
    {
        ...
        private static bool IsHiddenMember(MemberInfo mem)
        {
            ...
            return declaringType == Typeob.TObject || declaringType == Typeob.ScriptObject ||
                   (declaringType == Typeob.ArrayWrapper && mem.Name != "length");
        }
        ...
        private MemberInfo[] GetLocalMember(string name, BindingFlags bindingAttr, bool wrapMembers)
        {
            ...
            if (fieldInfo == null && isASubClass)
            {
                ...
                if ((array.Length == 0) && (bindingAttr & BindingFlags.Public) != BindingFlags.Default &&
                    (bindingAttr & BindingFlags.Instance) != BindingFlags.Default)
                {
                    var bindingFlags = (bindingAttr & BindingFlags.IgnoreCase) | BindingFlags.Public |
                    ...
                    {
                        array = TypeReflector.GetTypeReflectorFor(((NumberObject) this).baseType)
                            .GetMember(name, bindingFlags);
                    }
                    ...
                    {
                        array = TypeReflector.GetTypeReflectorFor(Typeob.String)
                            .GetMember(name, (bindingFlags | BindingFlags.Static) & ~BindingFlags.Instance);
                    }
                    else if (this is BooleanConstructor)
                    {
                        array = TypeReflector.GetTypeReflectorFor(Typeob.Boolean)
                            .GetMember(name, (bindingFlags | BindingFlags.Static) & ~BindingFlags.Instance);
                    }
        ...
        private MemberInfo[] GetMember(string name, BindingFlags bindingAttr, bool wrapMembers)
        {
            ...
            foreach (var memberInfo in array2)
            {
                ...
                FieldInfo fieldInfo2 = new TPrototypeField(parent,
                    new TGlobalField(this, name, LateBinding.GetMemberValue(parent, name, null, array),
                        FieldAttributes.FamANDAssem | FieldAttributes.Family | FieldAttributes.InitOnly));
        ...
        public override MemberInfo[] GetMembers(BindingFlags bindingAttr)
        {
            ...
            if (isASubClass)
            {
                ...
                while (i < num)
                {
                    ...
                    if (!memberInfo.DeclaringType.IsAssignableFrom(Typeob.TObject) &&
                        simpleHashtable[memberInfo.Name] == null)
                    {
            ...
            var array = WrapMembers(((IReflect) parent).GetMembers(bindingAttr & ~BindingFlags.NonPublic), parent,
                simpleHashtable2);
            var j = 0;
        ...
        internal override object GetMemberValue(string name)
        {
            ...
            else
            {
                ...
                {
                    return TProperty.GetGetMethod(property, false)
                        .Invoke(this, BindingFlags.SuppressChangeType, null, null, null);
                }
                try
                {
                    ...
                    if (method != null)
                    {
                        ...
                        if (declaringType != Typeob.TObject && declaringType != Typeob.ScriptObject &&
                            declaringType != Typeob.Object)
                        {
