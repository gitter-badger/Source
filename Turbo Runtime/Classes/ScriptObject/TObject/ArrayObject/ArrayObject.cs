    ...
    public class ArrayObject : TObject
    {
        ...
        private void SetLength(ulong newLength)
        {
            ...
            else
            {
                ...
                if (newLength > denseArrayLength && num <= denseArrayLength && newLength <= 100000uL &&
                    (newLength <= 128uL || newLength <= num*2u))
                {
        ...
        internal virtual void Splice(uint start, uint deleteCount, object[] args, ArrayObject outArray, uint oldLength,
            uint newLength)
        {
            ...
            if (oldLength > 0u)
            {
                Copy(denseArray, (int) (start + deleteCount), denseArray, (int) (start + (uint) args.Length),
                    (int) (oldLength - start - deleteCount));
            }
            if (args != null)
            {
                var num = args.Length;
        ...
        protected void SpliceSlowly(uint start, uint deleteCount, object[] args, ArrayObject outArray, uint oldLength,
            uint newLength)
        {
            ...
            else
            {
                ...
                {
                    SetValueAtIndex(num4 + start + (uint) args.Length - 1u,
                        GetValueAtIndex(num4 + start + deleteCount - 1u));
                }
            }
