			...
			if (fieldInfo != null)
			{
				...
				if (fieldInfo.IsLiteral)
				{
					...
					if (value is FunctionObject)
					{
						...
						{
							((TMemberField)fieldInfo).AddOverloadedMembers(memberInfoList, this, bindingAttr | BindingFlags.DeclaredOnly);
							goto IL_135;
						}
			...
			while (enumerator.MoveNext())
			{
				...
				if (fieldInfo.IsLiteral && fieldInfo is TMemberField)
				{
					...
					if ((value = ((TMemberField)fieldInfo).value) is FunctionObject)
					{
						...
						{
							((TMemberField)fieldInfo).AddOverloadedMembers(memberInfoList, this, BindingFlags.DeclaredOnly | BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic);
						}
					}
