	...
	public abstract class ScriptObject : IReflect
	{
		...
		public virtual object InvokeMember(string name, BindingFlags invokeAttr, Binder binder, object target, object[] args, ParameterModifier[] modifiers, CultureInfo locale, string[] namedParameters)
		{
			...
			if ((invokeAttr & BindingFlags.CreateInstance) != BindingFlags.Default)
			{
				...
				return new LateBinding(name, this).Call(binder, args, modifiers, locale, namedParameters, true, false, engine);
			}
		    if (name == null)
		    {
		    ...
		    if ((invokeAttr & (BindingFlags.InvokeMethod | BindingFlags.GetField | BindingFlags.GetProperty)) != BindingFlags.Default)
		    {
		        ...
		        if (!flag2)
		        {
		            if ((args != null && args.Length != 0) ||
		                ...
		                return new LateBinding(name, this).Call(binder, args, modifiers, locale, namedParameters, false, false,
		                    engine);
		            var memberValue = GetMemberValue(name);
		            ...
		            return new LateBinding(name, this).Call(binder, args, modifiers, locale, namedParameters, false, false, engine);
		        }
		...
		{
            LateBinding.SetMemberValue(this, name, value, LateBinding.SelectMember(GetMember(name, BindingFlags.Instance | BindingFlags.Public)));
		}
