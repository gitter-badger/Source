...
namespace Turbo.Runtime
{
	...
	internal sealed class TCodeGenerator : CodeCompiler
	{
		...
		protected override string CmdArgsFromParameters(CompilerParameters options)
		{
			...
			foreach (var current in options.ReferencedAssemblies.Cast<string>().Where(current => hashtable[current] == null))
			{
		...
		private string GetBaseTypeOutput(string baseType)
		{
		    return baseType.Length == 0
		        ...
		        ? "ulong"
		        : (string.Compare(baseType, "System.Decimal", StringComparison.Ordinal) == 0
		        ? "decimal"
		        : (string.Compare(baseType, "System.Single", StringComparison.Ordinal) == 0
		        ? "float"
		        : (string.Compare(baseType, "System.Double", StringComparison.Ordinal) == 0
		        ? "double"
		        : (string.Compare(baseType, "System.Boolean", StringComparison.Ordinal) == 0
		        ? "boolean"
		        : (string.Compare(baseType, "System.Char", StringComparison.Ordinal) == 0
		        ? "char"
		...
		{
		    var text = typeRef.ArrayElementType != null ? GetTypeOutput(typeRef.ArrayElementType) : GetBaseTypeOutput(typeRef.BaseType);
		...
		protected override void OutputDirection(FieldDirection dir)
		{
			...
			case FieldDirection.Ref:
				if (!isArgumentList)
				{
					throw new Exception(TurboException.Localize("No parameter direction", CultureInfo.CurrentUICulture));
				}
				Output.Write("&");
				break;
			default:
