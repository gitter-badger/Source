...
namespace Turbo.Runtime
{
	internal class Class : AST
	{
		...
		private static void AddImplicitInterfaces(IReflect iface, IReflect[] explicitInterfaces, ArrayList implicitInterfaces)
		{
		...
		{
		    implicitDefaultConstructor = new FunctionObject(".ctor", new ParameterDeclaration[0], null, new Block(context),
		...
		private bool CanSee(MemberInfo member)
		{
			...
			if (memberType <= MemberTypes.Method)
			{
				if (memberType != MemberTypes.Event)
				{
					if (memberType == MemberTypes.Field)
					{
						...
						return fieldAttributes != FieldAttributes.Private && fieldAttributes != FieldAttributes.PrivateScope && fieldAttributes != FieldAttributes.FamANDAssem && (fieldAttributes != FieldAttributes.Assembly || IsInTheSamePackage(member));
					}
				    ...
				    return methodAttributes != MethodAttributes.Private && methodAttributes != MethodAttributes.PrivateScope && methodAttributes != MethodAttributes.FamANDAssem && (methodAttributes != MethodAttributes.Assembly || IsInTheSamePackage(member));
				}
			    ...
			    return methodAttributes2 != MethodAttributes.Private && methodAttributes2 != MethodAttributes.PrivateScope && methodAttributes2 != MethodAttributes.FamANDAssem && (methodAttributes2 != MethodAttributes.Assembly || IsInTheSamePackage(member));
			}
		    if (memberType != MemberTypes.Property)
		    {
		        ...
		        return typeAttributes != TypeAttributes.NestedPrivate && typeAttributes != TypeAttributes.NestedFamANDAssem && (typeAttributes != TypeAttributes.NestedAssembly || IsInTheSamePackage(member));
		    }
            ...
            return (methodBase.Attributes & MethodAttributes.MemberAccessMask) != MethodAttributes.Private && (methodBase.Attributes & MethodAttributes.MemberAccessMask) != MethodAttributes.PrivateScope && (methodBase.Attributes & MethodAttributes.MemberAccessMask) != MethodAttributes.FamANDAssem && ((methodBase.Attributes & MethodAttributes.MemberAccessMask) != MethodAttributes.Assembly || IsInTheSamePackage(member));
		}
		...
		private void CheckFieldDeclarationConsistency(TVariableField field)
		{
			...
			while (i < num)
			{
				...
				if (CanSee(memberInfo))
				{
					...
					field.originalContext.HandleError(TError.HidesParentMember, fullNameFor, IsInTheSameCompilationUnit(memberInfo));
					return;
				}
				i++;
			}
		...
		private void CheckMethodDeclarationConsistency(FunctionObject func)
		{
			...
			if (memberInfo != null)
			{
			    ...
			    var fullNameFor = GetFullNameFor(memberInfo);
			    func.funcContext.HandleError(TError.HidesParentMember, fullNameFor, IsInTheSameCompilationUnit(memberInfo));
			    return;
			}
			CheckThatMethodIsNotMarkedWithOverrideOrHide(func);
			if ((func.attributes & MethodAttributes.Final) != MethodAttributes.PrivateScope)
		...
		private void CheckMatchingMethodForConsistency(MethodInfo matchingMethod, FunctionObject func, int i, int n)
		{
			...
			if (func.noVersionSafeAttributeSpecified)
			{
				if (Engine.versionSafe)
				{
					...
					{
						func.funcContext.HandleError(TError.NewNotSpecifiedInMethodDeclaration, IsInTheSameCompilationUnit(matchingMethod));
						i = -1;
					}
				}
		...
		private static bool DefinedOnInterface(MethodInfo meth) => ((ClassScope)(meth as TFieldMethod)?.func.enclosing_scope)?.owner.isInterface ?? meth.DeclaringType.IsInterface;
	    ...
	    private void EmitILForINeedEngineMethods()
		{
			...
			var field = typeBuilder.DefineField("vsa Engine", Typeob.THPMainEngine, FieldAttributes.Private | FieldAttributes.NotSerialized);
			var methodBuilder = typeBuilder.DefineMethod("GetEngine", MethodAttributes.Private | MethodAttributes.Virtual, Typeob.THPMainEngine, null);
			...
			var methodBuilder2 = typeBuilder.DefineMethod("SetEngine", MethodAttributes.Private | MethodAttributes.Virtual, Typeob.Void, new[]
			{
		...
		private void GenerateGetEnumerator()
		{
			...
			var methodBuilder = expr_0B.DefineMethod("get enumerator", MethodAttributes.Private | MethodAttributes.Virtual, Typeob.IEnumerator, null);
		...
		private void GetDynamicElementFieldGetter(TypeBuilder classwriter)
		{
		    ...
		    getHashTableMethod = classwriter.DefineMethod("get dynamic table", MethodAttributes.Private, Typeob.SimpleHashtable, null);
		...
		internal MethodInfo GetDynamicElementIndexerGetter()
		{
		    ...
		    getItem = typeBuilder.DefineMethod("get_Item", MethodAttributes.FamANDAssem | MethodAttributes.Family | MethodAttributes.SpecialName, Typeob.Object, new[]
		    {
		        Typeob.String
		    });
		...
		internal MethodInfo GetDynamicElementIndexerSetter()
		{
		    ...
		    var typeBuilder = classob.GetTypeBuilder();
		    GetDynamicElementFieldGetter(typeBuilder);
		    setItem = typeBuilder.DefineMethod("set_Item", MethodAttributes.FamANDAssem | MethodAttributes.Family | MethodAttributes.SpecialName, Typeob.Void, new[]
		    {
		        Typeob.String,
		        Typeob.Object
		    });
		...
		{
			var typeBuilder = classob.GetTypeBuilder();
			var methodBuilder = deleteOpMethod = typeBuilder.DefineMethod("op_Delete", MethodAttributes.FamANDAssem | MethodAttributes.Family | MethodAttributes.Static | MethodAttributes.SpecialName, Typeob.Boolean, new[]
			{
				typeBuilder,
				Typeob.ArrayOfObject
			});
			methodBuilder.DefineParameter(2, ParameterAttributes.None, null).SetCustomAttribute(new CustomAttributeBuilder(Typeob.ParamArrayAttribute.GetConstructor(Type.EmptyTypes), new object[0]));
		...
		internal MemberInfo[] GetInterfaceMember(string name)
		{
			...
			if (isInterface)
			{
				var member = classob.GetMember(name, BindingFlags.DeclaredOnly | BindingFlags.Instance | BindingFlags.Public);
			...
			foreach (var member in array.Select(t => t.ToIReflect().GetMember(name, BindingFlags.DeclaredOnly | BindingFlags.Instance | BindingFlags.Public)).Where(member => member != null && member.Length != 0))
			{
			    return member;
			}
		...
		internal ConstructorInfo GetSuperConstructor(IReflect[] argIRs) => (superTypeExpression != null ? superTypeExpression.Evaluate() : Typeob.Object) is ClassScope ? TBinder.SelectConstructor(((ClassScope)(superTypeExpression != null ? superTypeExpression.Evaluate() : Typeob.Object)).constructors, argIRs) : TBinder.SelectConstructor(((Type)(superTypeExpression != null ? superTypeExpression.Evaluate() : Typeob.Object)).GetConstructors(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic), argIRs);
		...
		internal virtual Type GetTypeBuilderOrEnumBuilder()
		{
			...
			{
				typeBuilder.SetCustomAttribute(new CustomAttributeBuilder(CompilerGlobals.clsCompliantAttributeCtor, new object[]
				{
					true
				}));
			}
			...
			if (generateCodeForDynamicElement)
			{
				typeBuilder.SetCustomAttribute(new CustomAttributeBuilder(CompilerGlobals.defaultMemberAttributeCtor, new object[]
				{
					"Item"
				}));
			}
			...
			while (k < num3)
			{
				...
				if (jSMemberField.IsLiteral)
				{
					...
					if (value is TProperty)
					{
						...
						var propertyBuilder = jSProperty.metaData = typeBuilder.DefineProperty(jSMemberField.Name, jSProperty.Attributes, jSProperty.PropertyType, array2);
						if (jSProperty.getter != null)
						{
						    ...
						    propertyBuilder.SetGetMethod((MethodBuilder)jSProperty.getter.GetMethodInfo(compilerGlobals));
						}
						if (jSProperty.setter != null)
						{
						    ...
						    propertyBuilder.SetSetMethod((MethodBuilder)jSProperty.setter.GetMethodInfo(compilerGlobals));
						}
					}
					...
					{
						var fieldBuilder = typeBuilder.DefineField(jSMemberField.Name, jSMemberField.FieldType, jSMemberField.Attributes);
		...
		private bool IsInTheSamePackage(MemberInfo member) => (member is TMethod || member is TField) &&
		                                                      classob.GetPackage() ==
		                                                      (member is TMethod ? ((TMethod) member).GetPackage() : ((TField) member).GetPackage());
	    ...
	    protected bool NeedsToBeCheckedForCLSCompliance()
		{
			...
			if (clsCompliance == CLSComplianceSpec.NotAttributed && (attributes & TypeAttributes.Public) != TypeAttributes.NotPublic)
			{
		...
		{
			var member = classob.GetMember(name, BindingFlags.DeclaredOnly | BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic);
			if (member == null)
			{
	    ...
	    private void TranslateToCOMPlusClass()
		{
			...
			if (!isInterface)
			{
				...
				var methodBuilder = compilerGlobals.classwriter.DefineMethod(".init", MethodAttributes.Private, Typeob.Void, new Type[0]);
