...
namespace Turbo.Runtime
{
	internal class Call : AST
	{
		...
		internal override bool Delete()
		{
			...
			var methodInfo = type.GetMethod("op_Delete", BindingFlags.Static | BindingFlags.Public | BindingFlags.ExactBinding, null, new[]
			{
				type,
		...
		internal override object Evaluate()
		{
			...
			try
			{
				...
				{
					obj = LateBinding.CallValue(lateBinding.obj, array, isConstructor, inBrackets, Engine, callableExpression.GetObject2(), TBinder.ob, null, null);
				}
		...
		internal override AST PartiallyEvaluate()
		{
			...
			if (inBrackets && AllParamsAreMissing())
			{
				...
				return new ConstantWrapper(new TypedArray(((TypeExpression)new TypeExpression(func).PartiallyEvaluate()).ToIReflect(), args.count + 1), context);
			}
		    ...
		    if (binding.member is Type)
		    {
		        ...
		        if (constantWrapper != null)
		        {
		            try
		            {
		                ...
		                {
		                    result = new ConstantWrapper(Convert.CoerceT(constantWrapper.context.GetCode(), type, true), context);
		                    return result;
		                }
		    ...
		    else if (binding.member is TVariableField)
		    {
		        ...
		        if (jSVariableField.value is ClassScope)
		        {
		            ...
		            {
		                if (!Convert.IsPromotableTo(array[0], underlyingTypeIfEnum) && !Convert.IsPromotableTo(underlyingTypeIfEnum, array[0]) && (!ReferenceEquals(array[0], Typeob.String) || underlyingTypeIfEnum == classScope))
		                {
		                    args[0].context.HandleError(TError.ImpossibleConversion);
		                }
		            }
		...
		internal override void TranslateToILDelete(ILGenerator il, Type rtype)
		{
			...
			if (func is Binding)
			{
				...
				{
					methodInfo = reflect.GetMethod("op_Delete", BindingFlags.Static | BindingFlags.Public | BindingFlags.ExactBinding, null, new[]
					{
						type,
						Typeob.ArrayOfObject
					}, null);
