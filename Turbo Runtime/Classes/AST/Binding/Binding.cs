...
namespace Turbo.Runtime
{
	public abstract class Binding : AST
	{
		...
		internal static readonly ConstantWrapper ReflectionMissingCW = new ConstantWrapper(System.Reflection.Missing.Value, null);
		...
		private bool AccessibleConstructor()
		{
			...
			if ((constructorInfo is TConstructor && ((TConstructor)member).GetClassScope().owner.isAbstract) || (!(constructorInfo is TConstructor) && constructorInfo.DeclaringType.IsAbstract))
			{
			...
			if (constructorInfo is TConstructor && ((TConstructor)constructorInfo).IsAccessibleFrom(Globals.ScopeStack.Peek()))
			{
				return true;
			}
			if (giveErrors)
		...
		private bool AccessibleField(bool checkWritable)
		{
			...
			if (!fieldInfo.IsPublic)
			{
				...
				{
					if ((!fieldInfo.IsFamily && !fieldInfo.IsFamilyOrAssembly) || !InsideClassThatExtends(Globals.ScopeStack.Peek(), fieldInfo.ReflectedType))
					{
						if (giveErrors)
						{
							context.HandleError(TError.NotAccessible, isFullyResolved);
		...
		private bool AccessibleMethod(MethodInfo meth)
		{
			...
			if (!meth.IsPublic)
			{
				...
				{
					if ((meth.IsFamily || meth.IsFamilyOrAssembly) && InsideClassThatExtends(Globals.ScopeStack.Peek(), meth.ReflectedType))
					{
						return true;
					}
					if (giveErrors)
		...
		internal static bool AssignmentCompatible(IReflect lhir, AST rhexpr, IReflect rhir, bool reportError)
		{
			if (rhexpr is ConstantWrapper)
			{
			    ...
			    if (classScope != null)
			    {
			        ...
			        if (enumDeclaration != null)
			        {
			            ...
			            {
			                var field = classScope.GetField((string)constantWrapper.value, BindingFlags.Static | BindingFlags.Public);
			                if (field == null)
			                {
			                    return false;
			                }
			                enumDeclaration.PartiallyEvaluate();
			                constantWrapper.value = new DeclaredEnumValue(((TMemberField)field).value, field.Name, classScope);
			            }
			    ...
			    else if (lhir is Type)
			    {
			        ...
			        if (type.IsEnum)
			        {
			            ...
			            {
			                var field2 = type.GetField((string)constantWrapper2.value, BindingFlags.Static | BindingFlags.Public);
			                if (field2 == null)
			                {
			                    return false;
			                }
			...
			if (lhir is Type && Typeob.Delegate.IsAssignableFrom((Type)lhir) && ReferenceEquals(rhir, Typeob.ScriptFunction) && rhexpr is Binding && ((Binding)rhexpr).IsCompatibleWithDelegate((Type)lhir))
			{
				return true;
			}
			...
			if (ReferenceEquals(rhir, Typeob.String) && (ReferenceEquals(lhir, Typeob.Boolean) || Convert.IsPrimitiveNumericType(lhir)))
			{
				if (reportError)
				{
			...
			if ((ReferenceEquals(lhir, Typeob.Char) && ReferenceEquals(rhir, Typeob.String)) || Convert.IsPromotableTo(lhir, rhir) || (Convert.IsPrimitiveNumericType(lhir) && Convert.IsPrimitiveNumericType(rhir)))
			{
				if (reportError)
				{
	    ...
	    internal static bool CheckParameters(ParameterInfo[] pars, IReflect[] argIRs, ASTList argAST, Context ctx, int offset = 0, bool defaultIsUndefined = false, bool reportError = true)
		{
			...
			while (i < num)
			{
				...
				var rhir = argIRs[i];
				if (i == num - 1 && ((reflect2 is Type && Typeob.Array.IsAssignableFrom((Type)reflect2)) || reflect2 is TypedArray) && CustomAttribute.IsDefined(pars[i + offset], typeof(ParamArrayAttribute), false))
				{
	        ...
	        if (offset != 0 || num >= num2 || defaultIsUndefined) return true;
		    {
		        for (var k = num; k < num2; k++)
		        {
		            ...
		            if (k >= num2 - 1 && CustomAttribute.IsDefined(pars[k], typeof (ParamArrayAttribute), false)) continue;
		            if (reportError)
		            {
		...
		internal override object Evaluate()
		{
		    ...
		    return type == Typeob.GlobalObject ||
		           (type != null && type != Typeob.StringObject && type != Typeob.NumberObject && type != Typeob.BooleanObject &&
	    ...
	    internal virtual IReflect InferTypeOfCall(TField inference_target, bool isConstructor)
		{
	        ...
	        var obj = memberInfo is TVariableField ? ((TVariableField)memberInfo).value : TypeReferences.GetConstantValue((FieldInfo)memberInfo);
		...
		private int PlaceValuesForHiddenParametersOnStack(ILGenerator il, MethodInfo meth, IReadOnlyList<ParameterInfo> pars)
		{
			var num = 0;
		...
		private bool ParameterlessPropertyValueIsCallable(MethodInfo meth, ASTList args, IReflect[] argIRs, bool constructor, bool brackets)
		{
		    ...
		    if ((meth as TWrappedMethod)?.GetWrappedObject() is GlobalObject || argIRs.Length != 0 || (!(meth is TMethod) && Typeob.ScriptFunction.IsAssignableFrom(meth.ReturnType)))
		    {
		...
		internal static void PlaceArgumentsOnStack(ILGenerator il, ParameterInfo[] pars, ASTList args, int offset, int rhoffset, AST missing)
		{
			...
			var flag = num2 > 0 && CustomAttribute.IsDefined(pars[num2 - 1], typeof(ParamArrayAttribute), false) && (count != num2 || !Convert.IsArrayType(args[count - 1].InferType(null)));
			...
			else
			{
				...
				if (!(this is CallableExpression) && !(constructor & brackets))
				{
					...
					catch (AmbiguousMatchException)
					{
						...
						if (methodInfo2 != null && ParameterlessPropertyValueIsCallable(methodInfo2, args, argIRs, false, brackets))
						{
							return;
						}
			    ...
			    if (CustomAttribute.IsDefined(memberInfo, typeof(TFunctionAttribute), false) && !(defaultMember is PropertyInfo))
			    {
			        var num = 0;
			        var attributeValue = ((TFunctionAttribute)CustomAttribute.GetCustomAttributes(memberInfo, typeof(TFunctionAttribute), false)[0]).attributeValue;
			        if ((constructor && !(memberInfo is ConstructorInfo)) || (attributeValue & TFunctionAttributeEnum.HasArguments) != TFunctionAttributeEnum.None)
			        {
			        ...
			        if (!CheckParameters(((MethodBase)memberInfo).GetParameters(), argIRs, args, context, num, true, isFullyResolved))
			        {
			            member = null;
			        }
			    }
			    else
			    {
			        ...
			        if (constructor && memberInfo is ConstructorInfo && !(memberInfo is TConstructor) && Typeob.Delegate.IsAssignableFrom(memberInfo.DeclaringType))
			        {
			            context.HandleError(TError.DelegatesShouldNotBeExplicitlyConstructed);
			            member = null;
			            return;
			        }
			        if (!CheckParameters(((MethodBase)memberInfo).GetParameters(), argIRs, args, context, 0, false, isFullyResolved))
			        {
			            member = null;
			        }
			    }
		...
		internal override object ResolveCustomAttribute(ASTList args, IReflect[] argIRs)
		{
			...
			else
			{
				...
				if (constructorInfo != null)
				{
					...
					if (Typeob.Attribute.IsAssignableFrom(declaringType) && CustomAttribute.GetCustomAttributes(declaringType, typeof(AttributeUsageAttribute), false).Length != 0)
					{
		...
		private MemberInfo ResolveOtherKindOfCall(ASTList argList, IReflect[] argIRs, bool constructor, bool brackets)
		{
			...
			if (memberInfo2 is PropertyInfo && !(memberInfo2 is TProperty) && memberInfo2.DeclaringType == Typeob.GlobalObject)
			{
				...
				{
					var method = propertyType.GetMethod("CreateInstance", BindingFlags.DeclaredOnly | BindingFlags.Instance | BindingFlags.Public);
					if (method != null)
					{
			...
			if (callableExpression != null)
			{
				...
				{
					memberInfo2 = new TGlobalField(null, null, constantWrapper.value, FieldAttributes.FamANDAssem | FieldAttributes.Family | FieldAttributes.Literal);
				}
			}
		    ...
		    if (memberInfo2 is FieldInfo && ((FieldInfo)memberInfo2).IsLiteral)
		    {
		        ...
		        var obj = memberInfo2 is TVariableField ? ((TVariableField)memberInfo2).value : TypeReferences.GetConstantValue((FieldInfo)memberInfo2);
		        if (obj is ClassScope || obj is Type)
		        {
		            if (constructor)
		            {
		                if (brackets)
		                {
		                    ...
		                    while (j < num2)
		                    {
		                        if (!ReferenceEquals(argIRs[j], Typeob.Object) && !Convert.IsPrimitiveNumericType(argIRs[j]))
		                        {
		                ...
		                members = obj is ClassScope ? ((ClassScope)obj).constructors : ((Type)obj).GetConstructors(BindingFlags.Instance | BindingFlags.Public);
		                if (members == null || members.Length == 0)
		                {
		                    context.HandleError(TError.NoConstructor);
		                    member = null;
		    ...
		    if (!brackets && ((type2 != null && Typeob.ScriptFunction.IsAssignableFrom(type2)) || reflect is ScriptFunction))
		    {
		        ...
		        memberInfo = member = type2.GetMethod(constructor ? "CreateInstance" : "Invoke", BindingFlags.DeclaredOnly | BindingFlags.Instance | BindingFlags.Public);
		        return memberInfo;
		    }
		    ...
		    if (ReferenceEquals(reflect, Typeob.Object) || ((reflect is ScriptObject & brackets) && !(reflect is ClassScope)))
		    {
		        return memberInfo2;
		    }
		    if (reflect is TypedArray || (reflect is Type && ((Type)reflect).IsArray))
		    {
		        ...
		        if (constructor)
		        {
		            ...
		            if (!ReferenceEquals(reflect, Typeob.Object) && !(reflect is ClassScope) && (!(reflect is Type) || Typeob.Type.IsAssignableFrom((Type)reflect) || Typeob.ScriptFunction.IsAssignableFrom((Type)reflect)))
		            {
		                goto IL_8EC;
		            }
		        }
		    ...
		    if (!constructor)
		    {
		        if (brackets && ReferenceEquals(reflect, Typeob.String) && (this.argIRs.Length != 1 || !Convert.IsPrimitiveNumericType(argIRs[0])))
		        {
		...
		internal override void SetValue(object value)
		{
			...
			else if (memberInfo is PropertyInfo)
			{
				...
				{
					TProperty.SetValue(propertyInfo, ((WithObject)((ClassScope)@object).GetParent()).contained_object, value, null);
					return;
				}
	    ...
	    private void TranslateToIL(ILGenerator il, Type rtype, bool preSet, bool preSetPlusGet, bool calledFromDelete = false)
		{
			if (member is FieldInfo)
			{
			    ...
			    if (preSet) return;
			    var obj = fieldInfo is TField ? ((TField)fieldInfo).GetMetaData() : (fieldInfo is TFieldInfo ? ((TFieldInfo)fieldInfo).field : fieldInfo);
		...
		internal override void TranslateToILCall(ILGenerator il, Type rtype, ASTList argList, bool construct, bool brackets)
		{
			...
			if (defaultMember != null)
			{
				if (isArrayConstructor)
				{
					...
					else
					{
						...
						{
							var arrayMethod = ((ModuleBuilder)type2.Module).GetArrayMethod(type2, ".ctor", CallingConventions.HasThis, Typeob.Void, array);
				...
				if (isArrayElementAccess)
				{
					...
					else
					{
						...
						var arrayMethod2 = compilerGlobals.module.GetArrayMethod(type3, "Get", CallingConventions.HasThis, elementType, array2);
			...
			if (memberInfo is MethodInfo)
			{
				...
				if (methodInfo is TFieldMethod || CustomAttribute.IsDefined(methodInfo, typeof(TFunctionAttribute), false))
				{
			...
			if (memberInfo is ConstructorInfo)
			{
				...
				{
					flag = (((TFunctionAttribute)CustomAttribute.GetCustomAttributes(constructorInfo, typeof(TFunctionAttribute), false)[0]).attributeValue & TFunctionAttributeEnum.IsInstanceNestedClassConstructor) > TFunctionAttributeEnum.None;
				}
				if (flag)
				{
			...
			else
			{
				if (memberInfo is FieldInfo && ((FieldInfo)memberInfo).IsLiteral)
				{
					...
					if (obj is Type || obj is ClassScope || obj is TypedArray)
					{
						...
						var type7 = obj is Type ? (Type)obj : (obj is ClassScope ? Convert.ToType((ClassScope)obj) : ((TypedArray)obj).ToType());
		...
		internal override void TranslateToILSet(ILGenerator il, AST rhvalue)
		{
			if (isArrayElementAccess)
			{
				...
				var arrayMethod = compilerGlobals.module.GetArrayMethod(type, "Set", CallingConventions.HasThis, Typeob.Void, array);
			...
			else
			{
				...
				else
				{
					...
					if (!isNonVirtual && methodInfo2.IsVirtual && !methodInfo2.IsFinal && (!methodInfo2.ReflectedType.IsSealed || !methodInfo2.ReflectedType.IsValueType))
					{
		...
		private object TranslateToSpeculativeEarlyBoundSet(ILGenerator il, AST rhvalue)
		{
			...
			while (i < count)
			{
				...
				else if (memberInfo is PropertyInfo)
				{
					...
					if (propertyInfo.GetIndexParameters().Length == 0 && !((methodInfo = TProperty.GetSetMethod(propertyInfo, true)) == null))
					{
						goto IL_A8;
					}
				}
		...
		private object TranslateToSpeculativeEarlyBindings(ILGenerator il, Type rtype, bool getObjectFromLateBindingInstance)
		{
			...
			{
				var memberInfo = allKnownInstanceBindingsForThisName[i];
				if (memberInfo is FieldInfo || (memberInfo is PropertyInfo && ((PropertyInfo)memberInfo).GetIndexParameters().Length == 0 && !(TProperty.GetGetMethod((PropertyInfo)memberInfo, true) == null)))
				{
					member = memberInfo;
					if (Accessible(false))
					{
			    ...
			    try
			    {
			        ...
			        if (methodInfo != null)
			        {
			            if (CheckParameters(methodInfo.GetParameters(), argIRs, argList, context, 0, true, false))
			            {
			                ...
			                else if ((methodInfo as TWrappedMethod)?.obj is ClassScope && ((TWrappedMethod)methodInfo).GetPackage() == ((ClassScope)reflect).package)
			                {
			                    goto IL_285;
			                }
			                member = methodInfo;
		...
		private void WarnIfNotFullyResolved()
		{
			...
			for (var scriptObject = Globals.ScopeStack.Peek(); scriptObject != null; scriptObject = scriptObject.GetParent())
			{
			    ...
			    if (!(scriptObject is ActivationObject) || ((ActivationObject) scriptObject).isKnownAtCompileTime) continue;
