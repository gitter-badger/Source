	...
	public sealed class Try : AST
	{
		...
		internal Try(Context context, AST body, AST identifier, TypeExpression type, AST handler, AST finally_block, bool finallyHasControlFlowOutOfIt, Context tryEndContext) : base(context)
		{
			...
			if (identifier != null)
			{
				...
				if (field != null)
				{
					if (type == null && field is TVariableField && field.IsStatic && ((TVariableField)field).type == null && !field.IsLiteral && !field.IsInitOnly)
					{
						return;
					}
			    ...
			    var jSVariableField = handler_scope.AddNewField(identifier.ToString(), Missing.Value, FieldAttributes.Public);
