...
namespace Turbo.Runtime
{
	internal sealed class CustomAttribute : AST
	{
		...
		private bool CheckIfTargetOK(object caType)
		{
		    ...
		    var validOn = caType is Type ? ((AttributeUsageAttribute)GetCustomAttributes(caType as Type, typeof(AttributeUsageAttribute), true)[0]).ValidOn : ((ClassScope)caType).owner.validOn;
			object obj = target;
			...
			if (functionDeclaration != null)
			{
				...
				{
					if (functionDeclaration.enclosingProperty.getter == null || ((TFieldMethod)functionDeclaration.enclosingProperty.getter).func == functionDeclaration.func)
					{
		...
		internal object GetTypeIfAttributeHasToBeUnique() 
            => !(type is Type)
                ...
                && !((AttributeUsageAttribute) GetCustomAttributes(type as Type, typeof (AttributeUsageAttribute), false)[0]).AllowMultiple
		            ? type as Type
		            : null);
	    ...
	    internal override AST PartiallyEvaluate()
		{
			...
			foreach (var parameterInfo in array2)
			{
			    ...
			    {
			        var value = TypeReferences.GetDefaultParameterValue(parameterInfo) == System.Convert.DBNull ? Convert.Coerce(null, reflect2) : TypeReferences.GetDefaultParameterValue(parameterInfo);
			...
			while (l < count4)
			{
				...
				if (assign2.lhside is Lookup && (assign2.rhside is ConstantWrapper || (assign2.rhside is ArrayLiteral && ((ArrayLiteral)assign2.rhside).IsOkToUseInCustomAttribute())))
				{
					...
					IReflect reflect3;
					if (obj3 is ArrayObject || ((reflect3 = TypeOfArgument(obj3)) != null && !ReferenceEquals(reflect3, Typeob.Object)))
					{
			...
			try
			{
                if (type is Type && target is AssemblyCustomAttributeList)
				{
					if ((type as Type).FullName == "System.Reflection.AssemblyAlgorithmIdAttribute")
					{
						if (positionalArgValues.Count > 0)
						{
							Engine.Globals.assemblyHashAlgorithm = (AssemblyHashAlgorithm)Convert.CoerceT(positionalArgValues[0], typeof(AssemblyHashAlgorithm));
						}
						AST result = null;
						return result;
					}
					...
					if ((type as Type).FullName == "System.Reflection.AssemblyFlagsAttribute")
					{
						if (positionalArgValues.Count > 0)
						{
							Engine.Globals.assemblyFlags = (AssemblyFlags)((uint)Convert.CoerceT(positionalArgValues[0], typeof(uint)));
						}
						AST result = null;
						return result;
					}
					if ((type as Type).FullName == "System.Reflection.AssemblyKeyFileAttribute")
					{
						if (positionalArgValues.Count > 0)
						{
							...
							if (Engine.Globals.assemblyKeyFileName != null && Engine.Globals.assemblyKeyFileName.Length == 0)
							{
					...
					{
						Engine.isCLSCompliant = (args == null || args.count == 0 || Convert.ToBoolean(positionalArgValues[0], false));
						AST result = this;
						return result;
					}
				}
				...
				else
				{
					...
					else
					{
						...
						else
						{
						    ...
						    revision = vString[num3 + 1] == '*' ? SecondsSinceMidnight() : ushort.Parse(vString.Substring(num3 + 1, length - num3 - 1), CultureInfo.InvariantCulture);
						}
					}
				}
			}
		...
		private static ushort SecondsSinceMidnight() 
            => (ushort)(((DateTime.Now - DateTime.Today).Hours * 60 * 60 + (DateTime.Now - DateTime.Today).Minutes * 60 + (DateTime.Now - DateTime.Today).Seconds) / 2);
	    ...
	    private static object[] ExtractCustomAttribute(IEnumerable<CustomAttributeData> attributes, Type caType)
		{
			...
			foreach (var current in attributes)
			{
			    ...
			    {
			        caType.InvokeMember(current3.MemberInfo.Name, BindingFlags.Instance | BindingFlags.Public | BindingFlags.SetField | BindingFlags.SetProperty, null, obj, new[]
			        {
