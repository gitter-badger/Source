...
namespace Turbo.Runtime
{
	public sealed class NumericBinary : BinaryOp
	{
		...
		internal NumericBinary(Context context, AST operand1, AST operand2, TToken operatorTok) : base(context, operand1, operand2, operatorTok)
		{
		}
		...
		private static object DoOp(long x, long y, TToken operatorTok)
		{
		    ...
		    switch (operatorTok)
		    {
		        ...
		        case TToken.Divide:
		            return x/(double) y;
		        case TToken.Modulo:
		            return y == 0L ? double.NaN : (x%y != 0L ? x%y : (x >= 0L ? (y < 0L ? -0.0 : 0) : (y < 0L ? 0 : -0.0)));
		    }
		    throw new TurboException(TError.InternalError);
		}
		...
		internal override IReflect InferType(TField inference_target)
		{
			...
			if ((Convert.IsPrimitiveNumericTypeFitForDouble(type1) || Typeob.TObject.IsAssignableFrom(type1)) && (Convert.IsPrimitiveNumericTypeFitForDouble(loctype) || Typeob.TObject.IsAssignableFrom(loctype)))
			{
		...
		internal override void TranslateToIL(ILGenerator il, Type rtype)
		{
			if (metaData == null)
			{
				...
				if (Convert.IsPrimitiveNumericType(rtype) && Convert.IsPromotableTo(type1, rtype) && Convert.IsPromotableTo(loctype, rtype))
				{
					type = rtype;
				}
