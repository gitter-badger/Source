...
namespace Turbo.Runtime
{
	internal sealed class TProperty : PropertyInfo
	{
	    ...
	    internal bool GetterAndSetterAreConsistent()
		{
			...
			if (!((TFieldMethod)getter).func.ReturnType(null).Equals(((ParameterDeclaration)parameters2[num]).type.ToIReflect()))
			{
				return false;
			}
			for (var i = 0; i < num; i++)
	    ...
	    public override MethodInfo GetGetMethod(bool nonPublic)
	    {
	        ...
	        try
	        {
	            ...
	            var property = ((ClassScope)setter.obj).GetSuperType().GetProperty(name, bindingFlags, null, null, new Type[0], null);
