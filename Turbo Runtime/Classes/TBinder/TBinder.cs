...
namespace Turbo.Runtime
{
	...
	internal sealed class TBinder : Binder
	{
		...
		{
			const BindingFlags bindingAttr = BindingFlags.DeclaredOnly | BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public;
		...
		internal static MemberInfo Select(TypeReferences typeRefs, MemberInfo[] match, int matches, IReflect[] argIRs, MemberTypes memberType)
		{
			...
			for (var i = 0; i < matches; i++)
			{
			    ...
			    array[i] = (memberInfo as PropertyInfo)?.GetIndexParameters() ?? ((MethodBase) memberInfo).GetParameters();
			    num++;
			}
		...
		private static int SelectBest(TypeReferences typeRefs, IReadOnlyList<MemberInfo> match, int matches, IReadOnlyList<IReflect> argIRs, IList<ParameterInfo[]> fparams, IReadOnlyList<object[]> aparams, int candidates, int parameters)
		{
			if (candidates == 0)
			{
				return -1;
			...
			for (var j = 0; j < matches; j++)
			{
			    ...
			    if ((argIRs?.Count ?? aparams[j].Length) > num && (num == 0 || !CustomAttribute.IsDefined(array3[num - 1], typeof(ParamArrayAttribute), false)))
			    {
			        fparams[j] = null;
			        candidates--;
			    }
			...
			while (candidates > 1)
			{
				...
				for (var l = 0; l < matches; l++)
				{
				    ...
				    if (num6 > 0 && num2 >= num6 - 1 && CustomAttribute.IsDefined(array4[num6 - 1], typeof(ParamArrayAttribute), false) && !(reflect is TypedArray) && !ReferenceEquals(reflect, typeRefs.ArrayObject) && (!(reflect is Type) || !((Type)reflect).IsArray))
				    {
				    ...
				    else if (num2 < num6)
				    {
				        ...
				        reflect2 = arg_24A_0;
				        if (ReferenceEquals(reflect, typeRefs.Missing) && !(TypeReferences.GetDefaultParameterValue(parameterInfo3) is DBNull))
				        {
				            reflect = reflect2;
				            num5 = 1;
				        }
				    ...
				    else
				    {
				        ...
				        for (var m = 0; m < l; m++)
				        {
				            ...
				            if ((!flag2 || parameters > num2) && (flag2 || !flag || !FormalParamTypeIsObject(fparams[m][num2])))
				            {
				                array[m] = true;
				            }
				        }
	    ...
	    internal static ConstructorInfo SelectConstructor(TypeReferences typeRefs, MemberInfo[] match, ref object[] args, string[] namedParameters)
		{
	        ...
	        if (type == null)
	            return match.Length == 1
	                ? match[0] as ConstructorInfo
                    : (ConstructorInfo)Select(typeRefs, match, match.Length, ref args, namedParameters, MemberTypes.Constructor);
	    ...
	    internal static MethodInfo SelectMethod(TypeReferences typeRefs, MemberInfo[] match, ref object[] args, string[] namedParameters)
		{
			...
			var memberInfo = (num == 1) ? match[0] : Select(typeRefs, match, num, ref args, namedParameters, MemberTypes.Method);
	    ...
	    internal static MethodInfo SelectMethod(TypeReferences typeRefs, MemberInfo[] match, IReflect[] argIRs)
		{
			...
			var memberInfo = (match.Length == 1) ? match[0] : Select(typeRefs, match, match.Length, argIRs, MemberTypes.Method);
		    ...
		    return num == 0
		        ...
		        : (num == 1
		            ...
		            : (match[0].MemberType == MemberTypes.Constructor
		                ? (MethodBase) (ConstructorInfo) Select(Runtime.TypeRefs, match, num, types, MemberTypes.Constructor)
		                : (MethodInfo) Select(Runtime.TypeRefs, match, num, types, MemberTypes.Method)));
		}

	    private static MethodBase SelectMethodBase(TypeReferences typeRefs, MethodBase[] match, ref object[] args, string[] namedParameters)
		{
			if (match == null)
			{
				return null;
			...
			var methodBase = (MethodBase)Select(typeRefs, match, num, ref args, namedParameters, MemberTypes.Method) ??
			                 (MethodBase)Select(typeRefs, match, num, ref args, namedParameters, MemberTypes.Constructor);
	        return methodBase;
		}
		...
		internal static MethodInfo SelectOperator(MethodInfo op1, MethodInfo op2, Type t1, Type t2)
		{
			...
			var arg_B8_0 = TypeDistance(Globals.TypeRefs, array[0].ParameterType, t1) + TypeDistance(Globals.TypeRefs, array[1].ParameterType, t2);
			var num = TypeDistance(Globals.TypeRefs, array2[0].ParameterType, t1) + TypeDistance(Globals.TypeRefs, array2[1].ParameterType, t2);
		...
		private static int TypeDistance(TypeReferences typeRefs, Type formal, Type actual)
		{
                        ...
                        && (actual == TypeReferences.Array || typeRefs.ArrayObject.IsAssignableFrom(actual))
			                ...
			                : (typeCode2 == TypeCode.String
			                    ...
			                    : (actual == typeRefs.ScriptFunction && TypeReferences.Delegate.IsAssignableFrom(formal) 
                                    ? 19 
                                    : 100));
			    case TypeCode.DBNull: return !(formal == TypeReferences.Object) ? 1 : 0;
			...
			case TypeCode.Decimal:
				switch (typeCode2)
				{
				case TypeCode.Object: return !(formal == TypeReferences.Object) ? 100 : 12;
