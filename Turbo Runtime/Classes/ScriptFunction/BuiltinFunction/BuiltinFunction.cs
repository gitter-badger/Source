...
namespace Turbo.Runtime
{
	internal sealed class BuiltinFunction : ScriptFunction
	{
		...
		internal BuiltinFunction(string name, object obj, MethodInfo method, ScriptObject parent) : base(parent, name)
		{
			...
			var jSFunctionAttribute = customAttributes.Length != 0 ? (TFunctionAttribute)customAttributes[0] : new TFunctionAttribute(TFunctionAttributeEnum.None);
		...
		internal static object QuickCall(object[] args, object thisob, TBuiltin biFunc, MethodInfo method, THPMainEngine engine)
		{
			var n = args.Length;
			switch (biFunc)
			{
