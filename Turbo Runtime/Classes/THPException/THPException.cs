...
namespace Turbo.Runtime
{
    ...
    public sealed class THPException : ExternalException
    {
        ...
        public override string ToString()
        {
            if ("" != Message)
            {
                return string.Concat("Turbo.Runtime.THPException: ",
                    Enum.GetName(((ETHPError) HResult).GetType(), (ETHPError) HResult), " (0x",
                    string.Format(CultureInfo.InvariantCulture, "{0,8:X}", new object[]
                    {
                        HResult
                    }), "): ", Message);
            }
